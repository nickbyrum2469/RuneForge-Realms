// Focused 0.6.1 terrain fragment override. Keep shared camera/sky/HUD helpers in voxel_scene.hlsl,
// but replace only the grass/dirt/root material sampling and terrain PS entry point so this pass
// cannot accidentally redesign unrelated materials.
...
MaterialSample sampleSurface061(uint material, float3 p, float3 n) {
    ...
    if (material == 0u) {
        // Grass top: use an irregular cellular micro-field for color/cavity instead of a global
        // axis-aligned 22x22 texel grid. The physical 16x16 relief cells remain cubical, but the
        // color variation can no longer form straight lines across an entire meadow.
    ...
    if (material == 19u) {
        // Dedicated thin root material. This is intentionally simple: the geometry supplies the
        // branching silhouette, while the shader gives it a readable warm/dark fiber range.
    ...
    // Dirt/grass-side share one 16-ish micro-clod language. Rows are independently staggered and
    // columns receive a smaller vertical warp, so the side still reads as voxel pieces without
    // becoming a perfect brick/grid texture.
    ...
    // Grass-side: the soil stays the same constructed material, then an irregular short turf lip
    // occupies only the upper portion. Roots are no longer painted procedurally here; near-camera
    // root fibers are explicit geometry, preventing the double-root/glued-column look from 0.6.0.
...
float4 PSMain(VSOutput input) : SV_Target0 {
    ...
    // The reference has substantially deeper, more saturated greens and browns than the washed-out
    // 0.6.0 hardware capture. Correct only this material family here; this is not a global lighting
    // or exposure pass and therefore cannot silently retune stone, bark, water, UI, or character.
    if (soilFamily) {
