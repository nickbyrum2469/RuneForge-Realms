...
bool VulkanRenderer::createPipeline() {
    ...
    vertexStage.stage = VK_SHADER_STAGE_VERTEX_BIT; vertexStage.module = vertexModule; vertexStage.pName = "VSMain";
    VkPipelineShaderStageCreateInfo fragmentStage{VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO};
    fragmentStage.stage = VK_SHADER_STAGE_FRAGMENT_BIT; fragmentStage.module = fragmentModule; fragmentStage.pName = "PSMain";
    ...
    blend.attachmentCount = 1; blend.pAttachments = &colorAttachment;
    ...
    pipelineInfo.stageCount = static_cast<std::uint32_t>(stages.size()); pipelineInfo.pStages = stages.data();
    ...
    pipelineInfo.pMultisampleState = &multisample; pipelineInfo.pDepthStencilState = &depth;
    pipelineInfo.pColorBlendState = &blend; pipelineInfo.pDynamicState = &dynamic;
    ...
    if (result != VK_SUCCESS) { setError(L"Frontier graphics pipeline creation failed", result); return false; }
    return true;
}
...
void VulkanRenderer::recordCommandBuffer(VkCommandBuffer commandBuffer, std::uint32_t imageIndex) {
    ...
    render.renderPass = renderPass_; render.framebuffer = framebuffers_[imageIndex];
    render.renderArea.extent = swapchainExtent_;
    render.clearValueCount = static_cast<std::uint32_t>(clearValues.size()); render.pClearValues = clearValues.data();
    ...
    if (indexCount_ > 0) {
        ...
        vkCmdBindVertexBuffers(commandBuffer, 0, 1, &vertexBuffer_.buffer, &offset);
