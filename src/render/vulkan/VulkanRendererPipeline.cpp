...
bool VulkanRenderer::chunkVisible(world::ChunkCoord coord) const noexcept {
    constexpr float kHorizontalFov = 1.74532925f; // 100 degrees; padded by chunk bounds in culler.
    constexpr float kMaxDistance = static_cast<float>((world::FrontierWorld::streamingRetainRadius + 2) *
                                                       world::VoxelChunk::sizeX);
    return culling::ChunkVisibility::visible(coord, player_.eyePosition(), player_.lookDirection(),
                                             kMaxDistance, kHorizontalFov);
}
...
void VulkanRenderer::recordCommandBuffer(VkCommandBuffer commandBuffer, std::uint32_t imageIndex) {
    ...
    for (const auto& [coord, mesh] : chunkMeshes_) {
        if (mesh.indexCount == 0 || !chunkVisible(coord)) continue;
