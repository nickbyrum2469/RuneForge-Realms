...
bool VulkanRenderer::createSceneMesh() {
    ...
    (void)world_.takeDirtyChunks();
    (void)world_.takeUnloadedChunks();
...
void VulkanRenderer::queueDirtyChunkMeshes() {
    for (const world::ChunkCoord coord : world_.takeDirtyChunks()) {
        ...
        pendingChunkMeshes_.push_back(PendingChunkMesh{
            coord,
            revision,
            meshJobs_.submit([snapshot = *snapshot, coord]() mutable {
                world::VoxelMesh local = world::GreedyMesher::build(snapshot);
                world::VoxelMesh global;
                global.append(local, static_cast<float>(coord.x * world::VoxelChunk::sizeX), 0.0f,
...
void VulkanRenderer::pumpChunkMeshJobs() {
    ...
    for (auto it = pendingChunkMeshes_.begin(); it != pendingChunkMeshes_.end();) {
...
void VulkanRenderer::removeUnloadedChunkMeshes() {
    for (const world::ChunkCoord coord : world_.takeUnloadedChunks()) {
...
void VulkanRenderer::drawSceneMeshes(VkCommandBuffer commandBuffer) {
    const scene::ChunkCullInput cull{
        ...
        static_cast<float>((world::ChunkManager::prefetchRadius + 1) * world::VoxelChunk::sizeX),
    };
