...
void runSurfaceReliefTests() {
    ...
    for (int v = 0; v < SurfaceReliefField::resolution; ++v) {
        for (int u = 0; u < SurfaceReliefField::resolution; ++u) {
            ...
            assert(std::abs(cell.vegetationOffsetU) <= 0.021f);
            assert(std::abs(cell.vegetationOffsetV) <= 0.021f);
    ...
    // Same 4x4 ecological patch shares broad tendencies without cloning exact cells.
    ...
    for (int v = 0; v < SurfaceReliefField::resolution; ++v) {
        for (int u = 0; u < SurfaceReliefField::resolution; ++u) {
            ...
            assert(cell.heightOffset >= 0.0f && cell.heightOffset <= 0.060f);
