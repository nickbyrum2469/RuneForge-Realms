...
void runCollisionResolvedLocomotionRegression() {
    ...
    world::FrontierWorld testWorld;
    testWorld.generate(424242u);
    for (int x = -2; x <= 2; ++x) {
        for (int z = -2; z <= 4; ++z) {
            (void)testWorld.setBlock(x, 8, z, world::BlockId::Stone, false);
            for (int y = 9; y <= 12; ++y) {
                (void)testWorld.setBlock(x, y, z, world::BlockId::Air, false);
            }
        }
    }
    ...
    (void)testWorld.setBlock(0, 9, 1, world::BlockId::Stone, false);
    (void)testWorld.setBlock(0, 10, 1, world::BlockId::Stone, false);

    game::PlayerController player;
    player.spawn({0.5f, 9.0f, 0.5f}, 0.0f, 0.0f);
    player.setControl(game::MoveControl::Forward, true);
    for (int i = 0; i < 6; ++i) player.update(0.020f, testWorld);
    ...
    for (int i = 0; i < 8; ++i) player.update(0.020f, testWorld);
    assert(std::abs(player.horizontalTravelDistance() - blockedDistance) < 0.0001f);
    assert(player.actualHorizontalSpeed() < 0.001f);

    (void)testWorld.setBlock(0, 9, 1, world::BlockId::Air, false);
    (void)testWorld.setBlock(0, 10, 1, world::BlockId::Air, false);
    player.update(0.020f, testWorld);
